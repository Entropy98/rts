	...
	if(forestNum>data.numOfForests):#base case
		...
		for i in range(len(board)):
			...
			for j in range(len(board[0])):
				iterations+=1
				if(board[i][j]=='forest'):
					drawLoadBar(display,data,'Compiling Tree Sprites...',(iterations)/(len(board)**2))
...
def drawLoadBar(display,data,msg,progress):
	display.fill((242,242,242))
	loadmessage=data.font.render(msg+str(int(progress*100))+'%',1,(20,20,20))
	display.blit(loadmessage,(data.width*.4,data.height*.77))
	pygame.draw.rect(display,(140,140,140),(data.width*.1,data.height*.8,data.width*.8,10))
	pygame.draw.rect(display,(179,179,179),(data.width*.1,data.height*.8+2,data.width*.8*progress,6))
